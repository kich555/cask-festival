> **[개정] 2026-09-13** — Meta 개발자 콘솔의 권한 문제(앱 관리자인데도 "개발자 역할
> 권한이 부족합니다")로 인스타그램 자동 동기화는 **보류**했다. 아래의 GitHub Actions 크론,
> 동기화 스크립트, Instagram API 관련 내용은 **구현하지 않았다.**
>
> 실제 구현된 범위: `/brands` 페이지 + `src/content/brands2026.json` 수동 관리 +
> `/register` 숨김 처리. 데이터 형태는 나중에 자동화를 얹을 수 있도록 그대로 유지했다.
> 운영 방법은 `docs/참가업체_추가하는법.md` 참고.
...
- 국가코드는 ISO 3166-1 alpha-2 2글자 (`JP`, `KR`, `TW`, `US` 등). 스코틀랜드·아일랜드처럼
  ISO alpha-2가 없는 지역은 `GB`, `IE` 등 국가 단위 코드를 쓴다.
